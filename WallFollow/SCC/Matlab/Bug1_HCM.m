    ...
    classdef Bug1_HCM < Navigation_ColorChanged
        ...
        properties(Access=protected)
            ...
            Hit_k
            flag    % flag variable is just used to make it possible to plot the last hit 
        ...
        methods
            ...
            function bug = Bug1_HCM(varargin)
            ...
            function n = next(bug, robot, radius)
                ...
                if (bug.step == 2) %&& bug.flag == 0) % flag variable is just used to it possible to plot the last hit 
                                                   % point before skipping to step 2 and starting circle motion
                    % disp(bug.step) 
                    ...
                    if bug.k <= numcols(bug.edge)
                        ...
                        if bug.isoccupied(n)
                            ...
                            obstacleHit = n;
                            bug.edge = circleedgelist(robot', obstacleHit', radius);
                    ...
                    if (bug.flag == 1) && ((colnorm(robot-bug.H(bug.j,:)') < 7)) % Complete tunring around the obstacle
                        ...
                        if bug.closestPoint_k > round(bug.Hit_k/2)
                            ...
                            bug.edge = circleedgelist(n', robot',radius);
                ...
                if bug.step == 1 % Step1 i ona gora Step2 dan spriya qoymusham ki o charpdighi nuxtani da shiklda chahsin sora getsin Step2 ya
                    ...
                    if bug.isoccupied(robot + [dx; dy])
                        ...
                        % get a list of all the points around the obstacle
                        obstacleHit = robot + [dx; dy];
                        bug.edge = circleedgelist(robot', obstacleHit', radius);
                        bug.first_edge = bug.edge;

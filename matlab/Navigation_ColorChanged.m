%Navigation Navigation superclass %%%%%%% The only difference with original
%Navigation class (in rorbot toolbox for Bug2) is the color of obstacles - changed from red
%to gray in line 343
%
    ...
    methods
        ...
        % See also Navigation.plot, Navigation.plot_fg, brighten.
            ...
            if isempty(opt.distance) || all(all(~isfinite(opt.distance)))
                ...
                cmap = [1 1 1; 0.65 0.65 0.65];  % non obstacles are white %%%%%---- The second row determine obstacle`s color

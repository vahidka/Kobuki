...
public:
            ...
            for (int column = x - grid_numbers_in_radius; column <= x + grid_numbers_in_radius; column++) {
                ...
                if ((abs(row-y) + abs(column-x)) <= grid_numbers_in_radius+1) {
                    if (occupancy_grid[MAP_SIZE-row][column] != 1) {
                        //cout << " marking " << " row " << row << " column " << column << " as " << value << endl;
    ...
    void updateMap()
    {
        ...
        int x, y;
        //cout << "bumper: " << (int)data.bumper << endl;
    ...
    void checkMap() // check obstables in front for movement controls
    {
    ...
    void testMotion()
    {
        kobuki.setBaseControl(0.0, -0.70);
        return;
    }
    ...
    void print(bool map)
    {
        ...
        // cout << "m0, target_y: " << target_y << " target_x: " << target_x << " desired_yaw: " << desired_yaw << " ";
        // cout <<  "yaw_error: " << yaw_error << " yaw_precision: " << yaw_precision << endl;
        if (!map) return;
        for (auto &row : occupancy_grid)
        {
            for (auto &column : row)
    ...
    void processMotion()
    {
        ...
        // std::cout << "Bumpers: " << (int)data.bumper << std::endl;
        checkMap();
        ...
        { // "wall following mode"
            ...
            { // if no hit or no obstacles on the front and right -> move for <Lenght>cm, then turn 90 degrees and look for the wall
                if (dx >= (length) && dth <= -ecl::pi * 0.5)
                {
                    // std::cout << "[Z] ";
                    dx = 0.0;
                    dth = 0.0;
                ...
                {
                    std::cout << "[R] " << dth << " " << dth*360.0/ecl::pi;
                    rotational_velocity = -ROTATION_SPEED * 2.0;
                }
                else
                {
                    // std::cout << "[L] ";
                ...
                // If completed the whole square but could not find the wall again,
                // recover and switch to go to goal mode. (avoid looping forever)
                if (corners_turned > 3)
                {
                    corners_turned = 0;
                    moving_state = ADJUST_HEADING;
